...
export const billingApi = {
  ...
  adjustBalance: async (userId: string, payload: BillingBalanceAdjustmentPayload): Promise<{ success: boolean; entry_no: string; account: BillingAccount }> => {
    const response = await apiClient.post(buildAdminApiPath(`/api/v1/admin/billing/accounts/${userId}/adjustments`), payload);
  ...
  listLedger: async (params?: { user_id?: string; page?: number; page_size?: number; entry_type?: number }): Promise<BillingLedgerListResponse> => {
    const response = await apiClient.get(buildAdminApiPath("/api/v1/admin/billing/ledger"), { params });
  ...
  listShortfalls: async (params?: { user_id?: string; page?: number; page_size?: number }): Promise<BillingShortfallListResponse> => {
    const response = await apiClient.get(buildAdminApiPath("/api/v1/admin/billing/shortfalls"), { params });
  ...
  reconcileShortfall: async (orderNo: string, payload?: BillingShortfallReconcilePayload): Promise<{ success: boolean; entry_no: string }> => {
    const response = await apiClient.post(buildAdminApiPath(`/api/v1/admin/billing/shortfalls/${orderNo}/reconcile`), payload ?? {});
  ...
  listUsageRecords: async (params?: { user_id?: string; page?: number; page_size?: number; direction?: number }): Promise<BillingUsageListResponse> => {
    const response = await apiClient.get(buildAdminApiPath("/api/v1/admin/billing/usage-records"), { params });
    return response.data as BillingUsageListResponse;
  },

  getPricing: async (): Promise<BillingPricing> => {
    const response = await apiClient.get(buildAdminApiPath("/api/v1/admin/billing/pricing"));
    return response.data as BillingPricing;
  },

  updatePricing: async (payload: BillingPricingUpdatePayload): Promise<BillingPricing> => {
    const response = await apiClient.put(buildAdminApiPath("/api/v1/admin/billing/pricing"), payload);
    return response.data as BillingPricing;
  },

  getWelcomeCreditSettings: async (): Promise<WelcomeCreditSettings> => {
    const response = await apiClient.get(buildAdminApiPath("/api/v1/admin/billing/welcome-credit"));
    return response.data as WelcomeCreditSettings;
  },

  updateWelcomeCreditSettings: async (payload: WelcomeCreditSettingsUpdatePayload): Promise<WelcomeCreditSettings> => {
    const response = await apiClient.put(buildAdminApiPath("/api/v1/admin/billing/welcome-credit"), payload);
    return response.data as WelcomeCreditSettings;
  },
};
